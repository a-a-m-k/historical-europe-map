...
vi.mock("@/context/useYearDataController", () => ({
  useYearDataController: () => ({
    ...
    isYearDataLoading: yearControllerState.isYearDataLoading,
...
vi.mock("@/components/layouts/MapStage", () => ({
  MapStage: (props: Record<string, unknown>) => {
    mapStageSpy(props);
    return <div data-testid="map-stage-year">{String(props.selectedYear)}</div>;
  },
}));
...
describe("MapLayout cross-flow integration", () => {
  beforeEach(() => {
    ...
    yearControllerState.isYearDataLoading = false;
  ...
  it("shows and hides historical loading overlay across loading transition", async () => {
    yearControllerState.isYearDataLoading = true;
    yearControllerState.filteredTowns = [];
