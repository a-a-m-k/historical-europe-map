...
describe("validateTownsData", () => {
  ...
  it("returns empty array when raw is not an array or envelope", () => {
    ...
    expect(validateTownsData({ schemaVersion: 99, towns: [] })).toEqual([]);
    expect(validateTownsData({ schemaVersion: 1, towns: "not array" })).toEqual(
      []
    );
  });

  it("accepts the versioned towns.json envelope", () => {
    const raw = {
      schemaVersion: 1,
      towns: [
        { ...validTown },
...
describe("towns.json shape (integration)", () => {
  it("loads and validates actual towns.json", async () => {
    ...
    expect(Array.isArray((raw as { towns: unknown }).towns)).toBe(true);
    const result = validateTownsData(raw);
