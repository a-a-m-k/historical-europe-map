...
describe("vite-plugin-critical", () => {
  ...
  it("skips extraction when SKIP_CRITICAL_CSS is set", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    const previous = process.env.SKIP_CRITICAL_CSS;
    process.env.SKIP_CRITICAL_CSS = "1";

    const { vitePluginCritical } =
      await import("../../../vite-plugin-critical");
    const plugin = vitePluginCritical({ base: tempRoot });
    const configResolved = resolveHook(plugin.configResolved);
    configResolved?.({
      build: { outDir: "dist" },
      base: "/",
    ...
    if (previous === undefined) {
      delete process.env.SKIP_CRITICAL_CSS;
    } else {
      process.env.SKIP_CRITICAL_CSS = previous;
    }
    warnSpy.mockRestore();
  });

  it("continues build when browser launch fails", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    criticalGenerateMock.mockRejectedValue(
