 ...
 * listens for global reset events, debounces refits after {@link useMapContainerResize} sees size
 * changes, runs one fit when `mapReady` first gets real dimensions, and refits when `isResizing`
 * goes true → false. Dispatches {@link dispatchMapCameraResetState} with whether `viewState` is
 * already at the reset target (for reset-button disabled state).
 *
