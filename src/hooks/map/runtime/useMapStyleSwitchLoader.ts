...
/**
 * Tracks **basemap / overlay style switch** readiness across one or two MapLibre maps.
 *
 * When `mapStyleMode` changes, increments an internal token, sets `isStyleSwitching` true, and
 * resets per-map idle flags. Completion requires overlay `idle` always; in **dark** split mode it
 * also waits for basemap `idle`. A timeout clears `isStyleSwitching` if `idle` never arrives (bad network).
 *
 ...
 * @param onFirstIdle - Optional callback the first time the overlay map idles.
 * @param styleSwitchTimeoutMs - Safety cap for a stuck switch spinner.
 * @returns `isStyleSwitching` for UI, plus `onOverlayIdle` / `onBasemapIdle` to wire to Map `onIdle`.
 */
