 ...
 * the `MapRef` after debounced **window** `resize` so GL dimensions stay correct. Returns
 * `{ width, height }` for {@link useMapViewConfig} / {@link useMapCameraLifecycle}.
 *
 * Attachment retries on `requestAnimationFrame` once so a late-mounted `containerRef` still gets observed.
 *
 * @param containerRef - The div that wraps the map canvas.
 * @param mapRef - Map ref; receives `resize()`.
