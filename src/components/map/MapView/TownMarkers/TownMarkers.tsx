 ...
 * Expects year-filtered towns from app state; only sorts by population for display order.
...
export const TownMarkers: React.FC<TownMarkersProps> = ({
  ...
  const sortedTowns = useMemo(() => {
    ...
    return [...towns].sort((a, b) => {
      const aPop = a.populationByYear?.[yearKey] ?? 0;
      const bPop = b.populationByYear?.[yearKey] ?? 0;
      return bPop - aPop;
    });
  }, [towns, selectedYear]);
