...
import { MARKER_EDGE_PADDING_PX } from "@/constants/mapGeoLayerStyle";
import {
  validateDimensions,
  getUIElementSizes,
  mercatorLatitude,
...
export function calculateFitZoom(
  ...
  const fitWidth = Math.max(
    mapWidth - 2 * MARKER_EDGE_PADDING_PX,
    MIN_MAP_DIMENSIONS.width
  );
  const fitHeight = Math.max(
    mapHeight - 2 * MARKER_EDGE_PADDING_PX,
  ...
  const latZoom = calculateZoomLevel(fitHeight, WORLD_DIM.height, latFraction);
  const lngZoom = calculateZoomLevel(fitWidth, WORLD_DIM.width, lngFraction);
